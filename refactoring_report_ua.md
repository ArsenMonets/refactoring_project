# Звіт про рефакторинг та тестування проекту "CubeField"

## Порівняльні метрики проєкту

| Метрика | Оригінальний код (Old) | Рефакторений код (New) | Результат |
| :--- | :---: | :---: | :--- |
| **Кількість файлів** | 1 | 12 | Покращення модульності |
| **Кількість функцій/методів** | 15 | ~54 | **+350%** (деталізація логіки) |
| **Сер. к-сть методів на клас** | 15 | ~6 | Дотримання принципу SRP |
| **SLOC (Рядки логіки)** | ~240 | ~680 | Розширення можливостей |
| **Max Cyclomatic Complexity** | 11 | 4 | **Знижено на 64%** |
| **Cognitive Complexity** | Висока | Дуже низька | Код легше читати |
| **Single Responsibility** | Порушено | Дотримано | Легко тестувати |
